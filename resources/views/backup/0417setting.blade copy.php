                        ...
                        <form class="d-flex flex-column" id="inputer" action="setting/domain" method="post">
                            ...
                            <div class="pt-2">
                                <label for="{{$formcontent->label}}" class="d-flex form-label">
                                    <span class="me-2">{{$formcontent->label}}</span>
                                    @if($formcontent->label=='Logo')
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="inlineRadioOptions" id="logo_url" checked />
                                        <label class="form-check-label" for="logo_url">
                                            網址
                                        </label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="inlineRadioOptions" id="logo_img" />
                                        <label class="form-check-label" for="logo_img">
                                            上傳圖片
                                        </label>
                                    </div>
                                    @elseif($formcontent->label=='API')
                                ...
                                <div class="@if($formcontent->label=='API') collapse @endif" id="{{$formcontent->id}}_container">
                                    ...
                                    @if($formcontent->label=='Logo')
                                    <input class="form-control visually-hidden" type="file" id="formFile" accept="image/*" disabled required>
                                    @endif
                                </div>
                            </div>
                            @endforeach
...
<script type="text/javascript">
    ...
    document.querySelectorAll('input[type=radio]').forEach(button => {
        button.addEventListener('click', () => {
            if (button.id === 'logo_img') {
                document.querySelector('#formFile').classList.remove('visually-hidden')
                document.querySelector('#formFile').removeAttribute('disabled')
                document.querySelector('#domainLogo').classList.add('visually-hidden')
                document.querySelector('#domainLogo').setAttribute('disabled', 'true')
            } else {
                document.querySelector('#formFile').classList.add('visually-hidden')
                document.querySelector('#formFile').setAttribute('disabled', 'true')
                document.querySelector('#domainLogo').classList.remove('visually-hidden')
                document.querySelector('#domainLogo').removeAttribute('disabled', 'true')
            }
        })
    });
    // if (apiSwitcher.ariaExpanded === 'false') {
