                ...
                <tr class="align-middle mx-auto">
                    @switch($collection['page'])
                        @case('sourcesites')
                            <td class="font-monospace">
                                <a class="text-decoration-none" href="{{ $content->domain_url }}">{{ $content->domain_name }}</a>
                            </td>
                        ...
                        @case('works')
                            ...
                            <td>
                                <a class="" href="{{ $content->git_repository_url }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="black"
                                        class="bi bi-git" viewBox="0 0 16 16">
                                        <path
                            ...
                            <td>
                                <button type="button" class="btn btn-secondary btn-sm" id="detial-button"
                                    data-bs-toggle="modal" data-bs-target="#detial" data-id="{{ $content->id }}">
                                    查看
                                </button>
                    ...
                    <td>
                        <button type="button" class="btn btn-outline-primary btn-sm" id="modify-button"
                            data-bs-toggle="modal" data-bs-target="#modify" data-id="{{ $content->id }}">編輯</button>
                        @include('setting.crud.modify')
                        <button type="button" class="btn btn-outline-danger btn-sm" id="delete-button"
                            data-bs-toggle="modal" data-bs-target="#delete" data-id="{{ $content->id }}">刪除</button>
